...
% the homework 6 for Econ 899 Excercise 2
% ***********************************************************************
clear
clc
close all
...
N = 50; % this is the transition path time
Na = 200;
...
theta_t = [theta0 * ones(21,1); thetaN * ones(N-21,1)];
...
c_K = abs(K_t - K_new); % the delta of capital
c_L = abs(L_t - L_new); % the delta of labor
c_Max = max(max(c_K,c_L));
diff = abs(c_Max);

% update K and L
K1 =  K_t + (K_new-K_t)./K_t;
L1 =  L_t + (L_new-L_t)./L_t;
